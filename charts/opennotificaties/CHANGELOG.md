...
- Increased the default `CELERY_WORKER_CONCURRENCY` from **4** to **100** to align with Open Notificaties 1.16.0 (the concurrency mode for the worker changed from processes to threads, reducing the memory footprint and making it cheaper to increase concurrency)
...
- [#188] Deleted possibility of running the automatic configuration with an init container, since this was broken and not usable.
- Updated the example in the values.yaml file to reflect the expected new OIDC config.
